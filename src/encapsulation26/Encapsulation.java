package encapsulation26;
/*Encapsulations example (Create package name ‘encapsulation26’ and create all
        below classes in this package)*/
public class Encapsulation {

    // private variables declared these can only be
    // accessed by public methods of class
    private String name;
    private int rollNo;
    private int age;
// get method for name to access private variable  name


    public String getName() {
        return name;
    }

    // set method for name to access private variable name
    public void setName(String name) {
        this.name = name;
    }
    //get method for roll to access private variable  rollNo


    public int getRollNo() {
        return rollNo;
    }
    //set mewthod for rollNo to access private variable rollNo

    public void setRollNo(int rollNo) {
        this.rollNo = rollNo;
    }

    //get method for age to acess private variable age

    public int getAge() {
        return age;
    }

    //set method for sge to acess private varible  age
    public void setAge(int age) {
        this.age = age;
    }


}